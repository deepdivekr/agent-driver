...
- **AI 연결 → 설정 대상 → 코딩 업무 전용**에서 **전역 AI 설정 사용**을 해제하면 코딩 계획·조언과 Codex/Claude CLI 실행 모델에 전용 선택을 우선 적용한다. 다시 체크해 저장하면 그 시점의 전역 설정을 상속한다. 처음에는 전역 설정을 그대로 사용하며, 다른 업무·Swarm의 설정은 바꾸지 않는다.
- 코딩 전용 API는 계획·조언용이다. 실제 코드 작업은 연결된 Codex·Claude CLI가 맡고 각 CLI 드롭다운의 모델을 사용한다. API 공급자와 주소가 전역과 같으면 전역 키를 중복 저장하지 않고 재사용한다. 다른 공급자·주소에는 해당 키를 자동 전달하지 않는다. 새 API 모델의 연결 검사는 요금 동의 후에만 수행한다.
- 코딩 계획·조언은 다음 호출, 고정 단계 CLI는 다음 단계, 대화형 Codex는 새 세션 연결부터 적용한다. 이미 연결된 대화는 재개 시에도 연결 당시 모델을 유지한다. 아직 코딩으로 분류되지 않은 첫 Work 정의는 전역 모델을 사용한다. 외부 Hermes/OpenClaw 작업자의 모델을 바꾸는 설정은 아니다.
...
런타임 DB 디렉터리의 `.connection/models.json`에 전역 선택·진행 단계·키를, `.connection/coding-models.json`에 코딩 전용 선택을 각각 원자적으로 저장한다. 상속 복귀는 코딩 전용 선택을 보존한 채 사용만 끈다. 비밀은 API 응답·로그·모델 입력에 넣지 않는다. UI 입력칸은 비밀번호 타입이며 저장 후 지운다. 공백 입력은 기존 키 유지, 삭제는 별도 체크박스다. revision 검사로 오래 열린 창의 덮어쓰기를 거부한다.
