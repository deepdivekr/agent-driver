...
test('refresh upgrades an older fast model to the newest listed one but keeps a deliberate larger choice',async()=>{
  const live=ids=>async()=>new Response(JSON.stringify({data:ids.map(id=>({id}))}),{status:200});
  const key='fixture-key-not-real-1234567890';
  assert.equal((await apiModelCatalog('openai',key,'',live(['gpt-5.6-luna','gpt-5.6-sol','gpt-6-sol','gpt-6-luna']),'gpt-5.6-luna')).selected,'gpt-6-luna');
  ...
  assert.equal(preferredFastModel('openai_compatible',['qwen3-30b','llama-4']),'qwen3-30b');
  assert.equal(selectedFastModel('openai',['gpt-6-luna'],'custom-finetune'),'gpt-6-luna','an unlisted choice moves to the listed fast model');
});
