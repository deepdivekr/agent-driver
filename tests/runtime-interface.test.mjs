...
test('runtime native schema v1 upgrades without discarding existing project/task/events',async t=>{
  ...
  const db=new DatabaseSync(path);assert.equal(db.prepare('SELECT version FROM schema_version').get().version,5);db.close();
});
test('runtime native C01 CLI and SDK stdio share capability semantics with independent tasks',{timeout:60000},async t=>{
  ...
  const catalog=await m.client.listTools();assert.equal(catalog.tools.length,26);assert.deepEqual(catalog.tools.map(t=>t.name).sort(),[...Object.keys(tools),'runtime_task_intake'].sort());
