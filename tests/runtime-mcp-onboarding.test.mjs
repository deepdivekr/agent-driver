...
test('runtime native setup activity SSE replays history and streams sanitized MCP progress',async t=>{
  ...
  for(let i=0;i<4&&!/Codex MCP 등록을 마쳤습니다/u.test(text);i++)text+=decoder.decode((await reader.read()).value);assert.match(text,/\[?MCP|Codex MCP 등록을 마쳤습니다/u);assert.doesNotMatch(text,new RegExp(secret));await reader.cancel();
});
