...
test('runtime Windows UIA contract separates observed inventory from exact reviewed invoke',async t=>{
  ...
  const request={computer_id:'windows-agent',task_id:'task-1',application_id:'sampleportal',window_automation_id:'main-window'},inventory=await client.observe(request);
  assert.equal(inventory.controls.length,2);const receipt=await client.invoke({computer_id:'windows-agent',task_id:'task-1',application_id:'sampleportal',window_automation_id:'main-window',capture_id:'capture-1',control_automation_id:'save-button',expected_control_type:'button',action:'invoke',review_id:'review-1',reviewed:true});
  ...
  await assert.rejects(client.invoke({computer_id:'windows-agent',task_id:'task-1',application_id:'sampleportal',window_automation_id:'main-window',capture_id:'capture-1',control_automation_id:'save-button',expected_control_type:'button',action:'invoke',review_id:'review-1',reviewed:false}),/WINDOWS_UIA_UNREVIEWED_ACTION/);
});
