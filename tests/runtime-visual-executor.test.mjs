...
test('runtime native visual pool isolates parallel browser storage and pages without preview capture',async t=>{
  ...
  assert.ok(surfaces.every(s=>s.preview_endpoint===''));
  ...
  await x.pool.release(surfaces[0].run_id,surfaces[0].worker_id);assert.equal(x.api.store.controlSurfaces(x.config.project.id).find(s=>s.id===surfaces[0].id).state,'closed');
  await x.pool.close();assert.ok(x.api.store.controlSurfaces(x.config.project.id).every(s=>s.state==='closed'));
});
