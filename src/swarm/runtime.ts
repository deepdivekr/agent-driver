...
export class SwarmRuntime{
  ...
  private async lease(runId:string,now:number,batch:boolean,singleFallback=false){
    ...
    const state={goal:snapshot.plan.goal,runnable:runnable.map(worker=>definitions.get(worker.id)),completed:Object.values(snapshot.workers).filter(worker=>worker.status==='succeeded').map(worker=>worker.id),reviews:snapshot.reviews},plane=await this.plane();let selected:string|null=null,eventId:string|null=null,decider:'code'|'jev'|'llm'='llm';
