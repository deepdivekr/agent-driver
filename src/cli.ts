...
import {runDecisionCli,decisionHelp} from './decision-plane/cli.js';
const args=process.argv.slice(2),command=args[0]??'help',options=new Map<string,string>();
try {
  if(await runDecisionCli(args)||await runHermesCli(args)||await runOnboardingCli(args)||await runVmCli(args)||await runSoakCli(args)||await runMaintenanceCli(args)||await runInterfaceCli(args)) { /* Shared API owns its lifecycle. */ }
  else {
