...
export function readTerminalOutput(store: TerminalStore, config: HostConfig, request: TerminalOutput) {
  ...
  const page = store.outputPage(config.project.id, request);
  const frames: Array<Record<string, unknown>> = [];
  let used = 0;
  if (page.rows.length) {
    // Pin the directory before opening the leaf. Never follow a replaced spool directory/leaf.
    const dir = openSync(join(dirname(config.dbPath), 'terminal-spool'), constants.O_RDONLY | constants.O_DIRECTORY | constants.O_NOFOLLOW);
    try {
      const fd = openSync(`/proc/self/fd/${dir}/${page.session.id}.jsonl`, constants.O_RDONLY | constants.O_NOFOLLOW | constants.O_NONBLOCK);
      try {
        const stat = fstatSync(fd);
        requireCondition(stat.isFile() && stat.nlink === 1 && stat.size >= page.session.spool_bytes, 'CLI_SPOOL_DURABILITY_GAP');
        for (const row of page.rows.slice(0, request.limit)) {
          ...
          while (read < bytes) {const n = readSync(fd, buffer, read, bytes - read, end - bytes + read); requireCondition(n > 0, 'CLI_SPOOL_DURABILITY_GAP'); read += n;}
          ...
          // Select known data fields only. Legacy unhashed records are never labelled verified.
          const frame = {type: data.type, subtype: typeof data.subtype === 'string' ? redact(data.subtype) : null,
            uuid: typeof data.uuid === 'string' ? redact(data.uuid) : null, session_id: page.session.cli_session_id,
            model: typeof data.model === 'string' ? redact(data.model) : null, result: typeof data.result === 'string' ? redact(data.result) : null};
