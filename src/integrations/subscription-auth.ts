...
// Claude counts as a subscription for a claude.ai login or a `claude setup-token` token (authMethod oauth_token),
// and only on first-party Anthropic with no API key source: an API key or cloud provider bills per use.
