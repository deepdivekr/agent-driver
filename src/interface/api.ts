...
export class RuntimeApi{
  ...
  constructor(readonly config:HostConfig){this.store=new RuntimeStore(config.dbPath);try{this.store.registerProject(config.project);}catch(e){this.store.close();throw e;}}
  ...
  async call(name:string,args:unknown):Promise<unknown>{
    ...
    switch(name){
      case 'runtime_health':return {health:this.config.environment==='fixture'?'ready':'degraded',verified_for_environment:false,environment:this.config.environment,execution_scope:'configured_fixture_only',model_execution_enabled:false};
        ...
        if(accepted.created){
          try{
            await new Promise<void>((resolve,reject)=>{
              const inherited=['PATH','HOME','USERPROFILE','SystemRoot','WINDIR','TEMP','TMP','TMPDIR','LANG','LC_ALL','PLAYWRIGHT_BROWSERS_PATH','XDG_CACHE_HOME'];
              const env=Object.fromEntries(inherited.flatMap(key=>process.env[key]===undefined?[]:[[key,process.env[key]!]]));
              const child=spawn(process.execPath,[fileURLToPath(new URL('./worker.js',import.meta.url)),this.config.path,accepted.task.id],{detached:true,windowsHide:true,stdio:'ignore',shell:false,env});
              child.once('error',reject);child.once('spawn',()=>{child.unref();resolve();});
            });
          }catch{this.store.pauseBeforeDispatch(accepted.task.id,'WORKER_START_FAILED');}
        }
      ...
      case 'runtime_recovery_status':return {tasks:this.store.tasks(this.config.project.id).filter(t=>['reconciliation_required','paused_dependency','ready_to_resume'].includes(t.status)).map(t=>this.store.outcome(t.id)),automatic_resume:false};
