...
export interface AdaptiveStepRecord {step:number;snapshot_sha256:string;state:string;operation:string;target_label:string|null;decider:'jev'|'llm';observe_ms:number;jev_ms:number;llm_ms:number;action_ms:number;verify_ms:number;result:string;}
...
export async function runAdaptivePack(options:AdaptiveRunOptions){
  ...
  try {
    ...
    for(let step=0;step<(options.maxSteps??35);step++){
      ...
      prior=snapshot;
      const decision=await decideAdaptiveStep(jev,task,spec,snapshot,history,options.minConfidence??0.5);jevCalls.push(decision.trace);
      ...
      if(action.operation==='DONE'){
        ...
        if(lastVerification.status==='MATCH'){await promoteAdaptiveSpec(specPath,designed.binding,hashJson(spec));status='succeeded';reason='INDEPENDENT_READBACK_MATCH';}
        else feedback='ADAPTIVE_COMPLETION_NOT_VERIFIED';
  ...
  const receipt={format:1,kind:'adaptive_readonly_pack',status,reason,task_sha256:hashJson(task),spec_sha256:specHash,cache_hit:cacheHit,design_ms:designMs,total_ms:Math.round(performance.now()-started),steps,jev_calls:jevCalls,llm_calls:llm.calls.slice(callsStart),verification:lastVerification,calibration:'uncalibrated_readonly_pilot',booking_or_payment_authorized:false};
