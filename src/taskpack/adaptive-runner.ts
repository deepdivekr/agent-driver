...
export async function runAdaptivePack(options:AdaptiveRunOptions){
  ...
  const plane=options.decisionPlane??new DecisionPlane({catalog:ADAPTIVE_DECISION_CATALOG,profile,primary:{id:'typesafe-jev',systemOne:(request,settings)=>jev.systemOne(request,settings)},...(shadow?{shadow}:{}),journal,shadow_sample_rate:options.shadowSampleRate??(shadow?0.1:0)});
  ...
  try {
    ...
    for(let step=0;step<(options.maxSteps??35);step++){
      ...
      prior=snapshot;
      const decision=await decideAdaptiveStep(jev,task,spec,snapshot,history,options.minConfidence??0.5,plane);jevCalls.push(decision.trace);
      let action:AdaptiveAction|null=decision.action,decider:'jev'|'llm'='jev',llmMs=0;
      const failure=feedback??(stagnation>=2?'ADAPTIVE_NO_PROGRESS':null)??decision.trace.reason??(action?.operation==='BLOCKED'?'ADAPTIVE_BLOCKED':null);
      ...
      const record:AdaptiveStepRecord={step:step+1,snapshot_sha256:hashJson(snapshot),state:action.state,operation:action.operation,target_label:snapshot.elements.find(element=>element.id===action!.target_id)?.label??null,decider,observe_ms:observeMs,jev_ms:decision.trace.elapsed_ms,llm_ms:llmMs,action_ms:0,verify_ms:0,result:'pending',decision_event_id:decision.event_id??null,shadow_disagreements:decision.shadow_disagreements??[]};
      if(action.operation==='DONE'){
        ...
        if(lastVerification.status==='MATCH'){await promoteAdaptiveSpec(specPath,designed.binding,hashJson(spec));if(record.decision_event_id)await plane.label(record.decision_event_id,{question_id:'complete',decision_id:'adaptive.complete',correct:true,expected:true,source:'readback',evidence_level:options.labelEvidenceLevel??'fixture'});status='succeeded';reason='INDEPENDENT_READBACK_MATCH';}
        else {if(record.decision_event_id)await plane.label(record.decision_event_id,{question_id:'complete',decision_id:'adaptive.complete',correct:false,expected:false,source:'readback',evidence_level:options.labelEvidenceLevel??'fixture'});feedback='ADAPTIVE_COMPLETION_NOT_VERIFIED';}
  ...
  const last=steps.at(-1),receipt={format:1,kind:'adaptive_readonly_pack',status,reason,task_sha256:hashJson(task),spec_sha256:specHash,cache_hit:cacheHit,design_ms:designMs,total_ms:Math.round(performance.now()-started),steps,jev_calls:jevCalls,llm_calls:llm.calls.slice(callsStart),verification:lastVerification,decision_plane:{catalog:plane.catalog.id,catalog_version:plane.catalog.version,profile:plane.profile.id,profile_status:plane.profile.status,shadow_events:steps.filter(step=>step.shadow_disagreements.length>0).length},handoff:status==='succeeded'?null:{to:'llm_or_parent_agent',reason,last_state:last?.state??'unobserved',last_operation:last?.operation??null,decision_event_id:last?.decision_event_id??null},booking_or_payment_authorized:false};
