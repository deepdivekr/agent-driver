...
export const settingsUpdate=z.object({revision:z.number().int().nonnegative(),selection:choice,onboarding_step:z.number().int().min(0).max(4),api_action:z.enum(['keep','replace','remove']).default('keep'),api_key:key.optional(),openai_action:z.enum(['keep','replace','remove']).default('keep'),openai_key:key.optional(),jev_action:z.enum(['keep','replace','remove']).default('keep'),jev_key:key.optional()}).strict();
...
function nextSettings(prior:ModelSettings|null,raw:unknown,base:NodeJS.ProcessEnv){
  ...
  const next:ModelSettings={format:1,revision:update.revision+1,selection:update.selection,onboarding_step:update.onboarding_step,...(prior?.api_key!==undefined?{api_key:prior.api_key}:prior?.openai_key!==undefined?{api_key:prior.openai_key}:{}),...(prior?.jev_key!==undefined?{jev_key:prior.jev_key}:{})};
  ...
  requireCondition(next.selection.mode!=='api'||Boolean(env.AGENT_DRIVER_API_KEY||env.OPENAI_API_KEY||env.ANTHROPIC_API_KEY||env.OPENROUTER_API_KEY),'MODEL_PROVIDER_CREDENTIAL_REQUIRED');
  requireCondition(next.selection.jev!=='on'||Boolean(env.TYPESAFE_API_KEY),'JEV_CREDENTIAL_REQUIRED');
  return next;
}
