    ...
    try{
      ...
      try{
        ...
        if(suffix==='settings/models'){
          ...
          const result=await probeStructuredModel(effectiveModelEnvironment(proposed,this.environment),this.fetcher),fingerprint=modelSettingsFingerprint(proposed,this.environment),token=randomBytes(24).toString('base64url');
          ...
          const proposed=previewModelSettings(this.path,clean,this.environment);let verified:ApiVerification|undefined;
          if(proposed.selection.mode==='api'){
            const probe=this.providerProbe,fingerprint=modelSettingsFingerprint(proposed,this.environment),prior=readModelSettings(this.path);
          ...
          const saved=saveModelSettings(this.path,clean,this.environment,verified);if(verified)this.providerProbe=null;
          const proposedBody=clean as {selection?:{mode?:unknown;jev?:unknown}};
          if(setupArea==='jev')await this.activity.record('jev','success',proposedBody.selection?.jev==='on'?'Jev 연결 설정을 저장했습니다.':'Jev 없이 LLM 판단으로 진행하도록 저장했습니다.');
