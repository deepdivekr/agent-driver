...
import {MIGRATION_1,MIGRATION_2,MIGRATION_3,MIGRATION_4,MIGRATION_5,MIGRATION_6,MIGRATION_7,MIGRATION_8} from '../store/migration.js';
...
export function schemaDigest(version:number) {
  requireCondition(version===6||version===7||version===8,'BACKUP_UNSUPPORTED_SCHEMA');
  if(!expected.has(version)){
    const db=new DatabaseSync(':memory:',{allowExtension:false});
    try{db.exec(MIGRATION_1);db.exec('INSERT INTO schema_version VALUES(1)');for(const migration of [MIGRATION_2,MIGRATION_3,MIGRATION_4,MIGRATION_5,MIGRATION_6,...(version>=7?[MIGRATION_7]:[]),...(version>=8?[MIGRATION_8]:[])])db.exec(migration);expected.set(version,digest(schema(db)));}finally{db.close();}
  }
  return expected.get(version)!;
}
export function inspectSnapshot(db:DatabaseSync) {
  db.exec('PRAGMA trusted_schema=OFF;');
  const actual=digest(schema(db));requireCondition(actual===schemaDigest(6)||actual===schemaDigest(7)||actual===schemaDigest(8),'BACKUP_SCHEMA_MISMATCH');
  const rows=db.prepare('SELECT version FROM schema_version').all();requireCondition(rows.length===1,'BACKUP_UNSUPPORTED_SCHEMA');
  const version=Number(rows[0]!.version);requireCondition(version===6||version===7||version===8,'BACKUP_UNSUPPORTED_SCHEMA');requireCondition(actual===schemaDigest(version),'BACKUP_SCHEMA_MISMATCH');
  ...
  return {schema_version:version as 6|7|8,schema_sha256:schemaDigest(version),instance_id:String(identity?.instance_id??'unobserved_legacy'),mode:String(identity?.mode??'unobserved_legacy'),
